# Contributing to SILC (Mobile)

So you'd like to contribute to the SILC Mobile codebase?  That's wonderful, we're excited to have your help :-)

Please do come and say hello in our [Slack chat](https://bantulogic.slack.com/messages/silc_mobile). You can get an invite by signing up at [Bantu Logic](https://www.bantulogic.org) or emailing [info@bantulogic.org](mailto:info@bantulogic.org).  We have [weekly meetings](https://www.bantulogic.org/events/silcmobile-planning) to coordinate our efforts and we try to do planning poker voting on tickets before starting work on them.  Feel free to join any [Bantu Logic daily scrum](https://www.bantulogic.org/events/) to ask questions, to listen in, or just say hi :-)
...
This describes how to contribute to SILC Mobile:  the tools we use to track and
...
First be sure that you've set up your development environment following all the steps
 in **[Setting Up for Development on SILC Mobile _(Project Set Up)_](https://github.com/bantulogic/SILCMobile/blob/develop/docs/project_setup.md)**


We use [Waffle](https://waffle.io/BantuLogic/SILCMobile) to manage our work on features, chores and bugfixes.
...
#### 1. Get the system working on your development environment:
   ...
   1. [install Android Studio on your dev environment (laptop or c9)](https://github.com/BantuLogic/SILCMobile/blob/develop/docs/project_setup.md)

   2. get tests passing (unit and integration tests in `spec/` and acceptance tests in `features`)

   3. check that the App can be run manually (on an emulator)

   4. (optional) install the App on your Android Device and ensure it runs there

#### 2. Look at what needs to be done on GitHub

* review [open PRs](https://github.com/BantuLogic/SILCMobile/pulls) on GitHub - leave comments or collaborate if interested


#### 3. Look at what needs to be done on [our waffle project board:](https://waffle.io/BantuLogic/SILCMobile)

  1. look through **[ready](https://waffle.io/BantuLogic/SILCMobile)** column - feel free to start work, but always interested to hear chat in slack, scrum wherever

  3. look through **[estimated](https://waffle.io/BantuLogic/SILCMobile)** column - feel free to ask about priority, as these are not prioritised

  4. look at **[backlog](https://waffle.io/BantuLogic/SILCMobile)** - if there is an interesting ticket get it voted on in a scrum or do an [ASYNC Vote](https://github.com/BantuLogic/BantuLogic/blob/master/ASYNC_VOTING.md) in Slack

##### Voting

  Items need to be voted on before work can start.  Voting happens in scrums or the weekly meeting (currently Fridays).  Note that even without the meetings you can get a vote on any issue you're thinking of working on by using the Async voting bot in the [#silc_mobile slack channel](https://bantulogic.slack.com/messages/C029E8G80/details/), using the following syntax: `/voter ISSUE NAME https://github.com/BantuLogic/BantuLogic/issues/number`.

e.g. 

```
/voter make a press-kit link in the footer https://github.com/BantuLogic/BantuLogic/issues/1738
```

More on how to handle a vote can be found at: https://github.com/BantuLogic/BantuLogic/blob/master/ASYNC_VOTING.md#automated-async-vote

## git and GitHub
Our **default working branch is `master`**.  We do work by creating branches off `develop` for new features and bugfixes.
...
Each developer will usually work with a [fork](https://help.github.com/articles/fork-a-repo/) of the [main repository on Bantu Logic](https://github.com/BantuLogic/SILCMobile). Before starting work on a new feature or bugfix, please ensure you have [synced your fork to upstream/develop](https://help.github.com/articles/syncing-a-fork/):
...
However the above is complicated and we are actively looking for some sort of testing solution that allows us to avoid the intermittent failing tests (maybe we need to upgrade capybara) and maybe dropping the whole caching approach is one way forward.  Please jump in our [Slack channel](https://bantulogic.slack.com/messages/C029E8G80/details/) and let us know what you think :-)


Pull Request Review
-------------------

Currently https://github.com/chaiwa-berian is project managing SILCMobile.  The project manager will review your pull request as soon as possible.  Usually the project manager will need to sign off in order to merge a pull request.
